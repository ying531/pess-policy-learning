...
for setting in range(5):
    ...
    if setting == 1: 
        ...
        SAVE_PATH = './results/results_MAB_clip_optimal.csv'
        SAVE_PATH_FREQ = './results/freq_MAB_clip_optimal.csv'
    # ======================
    ...
    if setting == 2: 
        ...
        SAVE_PATH = './results/results_MAB_clip_near-opt.csv'
        SAVE_PATH_FREQ = './results/freq_MAB_clip_near-opt.csv'
    # ====================== 
    ...
    if setting == 3: 
        ...
        ps = [0.2] * 5
        SAVE_PATH = './results/results_MAB_clip_uniform.csv'
        SAVE_PATH_FREQ = './results/freq_MAB_clip_uniform.csv'
    # ======================  
    ...
    for T in T_list:
        ...
        results = np.zeros((Nrep, 2*len(beta_list)+1))
        rewards = np.zeros((Nrep, 2*len(beta_list)+1))
        ...
        for seed in range(Nrep):
            ...
            # ======================
            # # pessimism after clipping """ 

            w_clip = np.minimum(np.ones(K) * 5, 1/np.array(ps)) 
            hat_mu_clip = np.array([np.mean(ys[:,w] *(ws==w)) * w_clip[w] for w in range(K) ])

            hat_V_clip = np.zeros(K)
            for w in range(K):
                hat_V_clip[w] = np.max([np.sqrt(T * np.mean(ws==w) * (w_clip[w]**2) ) / T, 
                                np.power(T * np.mean(ws==w) * (w_clip[w]**3), 1/4) / T])
                                
            pess_w_clip_list = []
            for ii in range(len(beta_list)):
                pess_w_clip_list.append(np.argmax(hat_mu_clip - beta_list[ii] * hat_V_clip))

            # ======================
            # # summarize results """
            all_w_list = [greedy_w] + pess_w_list + pess_w_clip_list
        ...
        sum_single = pd.DataFrame({"prob_correct": prob_correct, 
                                    ...
                                    "method_param": ['greedy'] + ["pess_"+str(beta) for beta in beta_list] + ["clip_"+str(beta) for beta in beta_list],
                                    "param": [0] + beta_list + beta_list,
                                    "method": ['greedy'] + ['pessimism'] * len(beta_list) + ['clip'] * len(beta_list),
                                    "T": T})
